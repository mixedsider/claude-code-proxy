...
3. **환경 변수 설정**:
   ...
   *   `PREFERRED_PROVIDER` (선택 사항): `openai` (기본값), `google`, `anthropic`, `ollama` 또는 `lm-studio`로 설정합니다. 이는 `haiku`/`sonnet` 매핑을 위한 기본 백엔드를 결정합니다.
   *   `BIG_MODEL` (선택 사항): `sonnet` 요청을 매핑할 모델입니다. 기본값은 `gpt-4.5-preview` (`PREFERRED_PROVIDER=openai`인 경우) 또는 `gemini-2.0-pro-exp-02-05`입니다.
   *   `SMALL_MODEL` (선택 사항): `haiku` 요청을 매핑할 모델입니다. 기본값은 `gpt-4o-mini` (`PREFERRED_PROVIDER=openai`인 경우) 또는 `gemini-2.0-flash`입니다.
   *   `OLLAMA_BASE_URL` / `LM_STUDIO_BASE_URL` (선택 사항): 로컬 엔진 사용 시 API 주소를 설정합니다.

   **매핑 로직:**
   - `PREFERRED_PROVIDER=openai` (기본값)인 경우, `haiku`/`sonnet`은 `openai/` 접두사가 붙은 `SMALL_MODEL`/`BIG_MODEL`로 매핑됩니다.
   - `PREFERRED_PROVIDER=google`인 경우, `haiku`/`sonnet`은 `gemini/` 접두사가 붙은 `SMALL_MODEL`/`BIG_MODEL`로 매핑됩니다.
   - `PREFERRED_PROVIDER=ollama` 또는 `lm-studio`인 경우, 해당 로컬 엔진의 모델로 매핑됩니다.
   - `PREFERRED_PROVIDER=anthropic`인 경우, Anthropic으로 직접 전달됩니다.
...
4. **서버 설치 및 실행**:
   ...
   **리눅스/macOS:**
   ...
   ./install.sh  # 환경 설정 (uv 설치 및 .env 생성)
   ./start.sh    # 서버 실행
   ```

   **윈도우 (PowerShell):**
   ```powershell
   .\install.ps1  # 환경 설정 (uv 설치 및 .env 생성)
   .\start.ps1    # 서버 실행
   ```

   *(참고: `install` 스크립트는 최초 1회만 실행하면 됩니다. 이후에는 `start` 스크립트만 사용하여 서버를 켤 수 있습니다.)*
...
제공된 테스트 스크립트로 서버 작동을 확인할 수 있습니다:
```bash
uv run python tests.py --proxy-only # 프록시 서버만 테스트
uv run python tests.py --test ollama --proxy-only # 특정 모델 테스트
```
